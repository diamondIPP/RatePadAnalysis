...
from helpers.draw import scale_multigraph, format_statbox


class PulserCollection(Analysis):

    def __init__(self, ana_collection):
        Analysis.__init__(self, testcampaign=ana_collection.TCString)
    ...
    def get_pulse_heights(self, corr=True, beam_on=True, redo=False):
        ...
        def f():
            ...
            for i, (key, ana) in enumerate(self.Analyses.items()):
    ...
    def get_pulse_height_graph(self, sigma=False, vs_time=False, corr=True, beam_on=True, redo=False, legend=True, show_flux=True):
        ...
        y_values = [dic['sigma' if sigma else 'ph'] for dic in ph.values()]
        x_values = [dic['time' if vs_time else 'flux'] for dic in ph.values()]
        g = Draw.make_tgrapherrors('g_pph', 'data', self.Draw.get_color(), marker_size=marker_size, x=x_values, y=y_values)
        g_first = Draw.make_tgrapherrors('g1', 'first run', marker=22, color=2, marker_size=marker_size, x=[x_values[0].n], y=[y_values[0].n])
        g_last = Draw.make_tgrapherrors('g2', 'last run', marker=23, color=2, marker_size=marker_size, x=[x_values[-1].n], y=[y_values[-1].n])
        graphs = [g] if vs_time else [g, g_first, g_last]
        leg = Draw.make_legend(.17, .35, nentries=3, x2=.4)
        ...
        if vs_time and show_flux:
            ...
            for i, (ana, x) in enumerate(zip(self.Analyses.values(), x_values)):
                y, ey = g.GetY()[i], g.GetErrorY(i)
                mg.GetListOfGraphs()[0].GetListOfFunctions().Add(Draw.tlatex(x.n, y + ey * 1.2, '{:1.0f}'.format(ana.get_flux().n), color=1, align=21, size=.02))
        return mg

    def draw_pulse_height(self, evts_per_bin=10000, redo=False, show=True, rel_t=True):
        """Draw time evolution of the pulse height"""
        graphs = [ana.Pulser.draw_pulse_height(evts_per_bin, show=False, redo=redo)[0] for ana in self.Analyses.values()]
        ...
        for g in graphs:
            ...
            for i in range(g.GetN()):
                ...
                i_bin += 1
            i_bin += 1
        format_histo(h1, x_tit='Time [hh:mm]', y_tit='Pulser Pulse Height [au]', y_off=.8, fill_color=Draw.FillColor, stats=0, y_range=[0, 105])
        set_time_axis(h1, off=self.Analysis.FirstAnalysis.Run.StartTime if rel_t else 0)
        self.Draw(h1, 'PulserPulseHeight{}'.format(evts_per_bin), show=show, draw_opt='hist', x=1.5, y=.75, lm=.065)
    ...
    def draw_pulse_heights(self, sigma=False, corr=True, beam_on=True, vs_time=False, do_fit=False, save_comb=True, show=True, redo=False):
        ...
        y_values = [mg.GetListOfGraphs()[0].GetY()[i] for i in range(mg.GetListOfGraphs()[0].GetN())]
        ...
        self.Draw(mg, 'Pulser{mean}{a}{b}'.format(mean='Sigma' if sigma else 'Mean', a=corr, b=beam_on), lm=.14, draw_opt='A', logx=False if vs_time else True, show=False)
        mg1 = mg.Clone()
        mg1.GetListOfGraphs()[0].SetLineColor(Draw.Colors[0])
        self.draw_legend()
        if do_fit:
        ...
        if save_comb:
            ...
            self.Draw.save_combined_pulse_heights(mg, mg1, ymin, show, name='CombinedPulserPulseHeights', pulser_leg=self.draw_legend)
            Draw.add(mg1)
        return mg
    ...
    def draw_scaled_pulse_heights(self, sigma=False, vs_time=False, redo=False, y_range=None, scale=1, show=True):
        ...
        self.Draw(mg, 'ScaledPulseHeights', show, lm=.14, draw_opt='a', logx=not vs_time, grid=vs_time, gridy=True, bm=.18)
        Draw.irradiation(make_irr_string(self.Analysis.selection.get_irradiation()))
        return mg

    def draw_legend(self):
        try:
            typ = self.Analysis.FirstAnalysis.Run.Info['pulser']
            ...
            leg = Draw.make_legend(.17, .88, nentries=3, margin=.05, clean=True, x2=.5)
    ...
    def draw_distributions(self, show=True, corr=True):
        ...
        legend = Draw.make_legend(nentries=self.Analysis.NRuns)
        c = Draw.canvas('Pulser Histos', 1, 1)
        c.SetRightMargin(.03)
        for i, ana in enumerate(self.Analyses.values()):
            ...
            format_histo(h, fill_color=0, stats=0, x_range=x_range, color=self.Draw.get_color(), lw=2)
            h.Scale(1 / h.GetMaximum())
            h.SetLineColor(self.Draw.get_color(self.Analysis.NRuns))
            h.SetLineWidth(2)
            h.Draw() if not i else h.Draw('same')
            legend.AddEntry(h, '{0:6.2f} kHz/cm^{{2}}'.format(ana.get_flux().n), 'l')
            Draw.add(h)
        legend.Draw()
        self.Draw.save_plots('AllPulserHistos{0}'.format('Uncorrected' if not corr else ''))
    ...
    def draw_all_pulse_heights(self, sigma=False, show=True):
        ...
        for i, gr in enumerate(graphs, 1):
            ...
            gr.Draw('a')
        Draw.add([graphs, c])
        self.Draw.save_plots('AllPulserOverview{0}'.format('Sigma' if sigma else 'Mean'), show)

    def draw_rate(self, evts_per_bin=1000, cut=None, rel_t=True, show=True):
        histos = [ana.Pulser.draw_rate(evts_per_bin, show=False, cut=cut, vs_time=True) for ana in self.Analyses.values()]
        ...
        for h in histos:
            for i in range(1, h.GetNbinsX() + 1):
                ...
                i_bin += 1
            i_bin += 1
        format_histo(h1, x_tit='Time [hh:mm]', y_tit='Pulser Rate [%]', y_off=.8, fill_color=Draw.FillColor, stats=0, y_range=[0, 105])
        set_time_axis(h1, off=self.Analysis.FirstAnalysis.Run.StartTime if rel_t else 0)
        self.Draw(h1, 'AllPulserRate', show=show, draw_opt='hist', x=1.5, y=.75, lm=.065)

    def draw_rates(self, show=True, vs_time=False, real=False):
        mode = self.Analysis.get_mode(vs_time)
        x_values, y_values = [], []
        self.PBar.start(self.Analysis.NRuns)
        for i, ana in enumerate(self.Analyses.values(), 1):
            ...
            self.PBar.update(i)
        g = Draw.make_tgrapherrors('g_pr', 'Pulser Fraction vs {mod} '.format(mod=mode), x=x_values, y=y_values, color=Draw.Colors[0], marker_size=2)
        format_histo(g, x_tit=self.Analysis.get_x_tit(vs_time), y_tit='Pulser Fraction [%]')
        self.Draw(g, 'PulserFraction{0}'.format(mode), show, logx=not vs_time, draw_opt='ap')
        return g
    ...
    def calc_all_errors(self):
        collimator_settings = set([(ana.Run.Info['fs11'], ana.Run.Info['fs13']) for key, ana in self.Analyses.items()])
        fits = {}
