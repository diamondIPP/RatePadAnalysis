...
from numpy import polyfit, pi, RankWarning, vectorize, size, split, repeat, argmax, insert, roll, cumsum, histogram
...
class PeakAnalysis(SubAnanlysis):
    ...
    def draw_n_bunch(self, b=0, y_range=None, show=True):
        ...
        format_statbox(entries=True)
        format_histo(h, x_tit='Time [ns]', y_tit='Number of Entries', y_off=1.3, fill_color=Draw.FillColor, y_range=y_range)
        self.Draw(h, lm=.12, show=show, w=1.5, h=0.75, logy=True)
        return times
    ...
    def draw_bunch_systematics(self, n=None, show=True):
        ...
        g = Draw.make_tgrapherrors('g', 'Bunch Systematics', x=arange(1, n_peaks.size + 1), y=n_peaks)
        format_histo(g, x_tit='Bunch after a Signal', y_tit='Average Number of Peaks', y_off=1.3)
        self.Draw(g, lm=.12, show=show)
    ...
    def draw_flux_vs_threshold(self, steps=20):
        ...
        g = Draw.make_tgrapherrors('gft', 'Flux vs. Peak Threshold', x=x, y=y)
        format_histo(g, x_tit='Peak Finding Threshold [mV]', y_tit='Flux [MHz/cm^{2}]', y_off=1.3)
        self.Draw(g, draw_opt='ap', lm=.12)
    ...
    def draw_peak_spacing(self, overlay=False, bin_size=.5, show=True):
        ...
        format_statbox(entries=True)
        format_histo(h, x_tit='Peak Distance [ns]', y_tit='Number of Entries', y_off=1.2, fill_color=Draw.FillColor)
        x, y = [None, None] if overlay else [1.5, .75]
        self.Draw(h, show=show, lm=.11, w=x, h=y)
    # endregion DRAW
    # ----------------------------------------
    ...
    def find_additional(self, h=None, scale=False, show=True):
        ...
        g = Draw.make_tgrapherrors('ga', 'Additional Peak Heights', x=(peaks[0] + self.StartAdditional) / 2., y=values[peaks[0]])
        format_statbox(fit=True)
        g.Fit('pol0', 'qs')
        self.Draw(g, show=show, w=1.5, h=.75, gridy=1)
    ...
    def draw_additional_disto(self, show=True):
        ...
        for h in hs:
            for v in self.find_additional(h):
                h0.Fill(v.n)
        format_histo(h0, x_tit='Peak Height', y_tit='Number of Entries', y_off=1.3, fill_color=Draw.FillColor)
        self.Draw(h0, show=show)
