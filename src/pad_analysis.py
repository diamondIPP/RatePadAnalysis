from ROOT import gRandom, TProfile2D, THStack, Long, TCut
from numpy import split, log2
...
class PadAnalysis(DUTAnalysis):
    ...
    def get_integral_names(self):
        return loads(self.Run.TreeConfig.get('Integral Names', 'Names'))
    ...
    def get_pulse_height(self, bin_size=None, cut=None, redo=False, corr=True, sig=None, sys_err=0, peaks=False):
        ...
        suffix = '{bins}{cor}_{c}'.format(bins=self.Bins.Size if bin_size is None else bin_size, cor=correction, reg=self.get_short_regint(sig), c=self.Cut(cut).GetName())
    ...
    def draw_efficiency_map(self, res=None, n_sigma=4, cut=None, show=True):
        ...
        e, y, x = self.get_root_vec(var=['({}>{})*100'.format(self.generate_signal_name(), thresh)] + list(self.Cut.get_track_vars(self.DUT.Number - 1, mm=True)), cut=cut_string)
    ...
    def make_signal_time_histos(self, bin_width=.2, signal_name=None, evnt_corr=False, off_corr=False, bin_corr=False, rel_t=False, show=True):
        ...
        format_statbox(entries=True, x=.83)
        ...
        self.Draw(h, 'SignalTime', show, lm=.12, draw_opt='colz', rm=.15)
        return h
    ...
    def draw_pulse_height(self, bin_size=None, cut=None, y_range=None, redo=False, corr=True, sig=None, rel_t=True, show=True, save=True, prnt=True):
        ...
        def f():
            ph, t = self.get_root_vec(var=[self.generate_signal_name(sig, corr), self.get_t_var()], cut=self.Cut(cut))
        ...
        self.Draw.save_plots('PulseHeight{}'.format(bin_size), show=show, save=save, prnt=prnt)
        return p, fit
    ...
    def draw_ph(self, bin_size=10000, y_range=None, rel_t=False, show=True):
        ...
        gr = Draw.make_tgrapherrors('hphl', 'Pulser Height Evolution')
        ...
        i = 0
        for xbin in range(2, h.GetNbinsX() + 1):  # first bin is always empty
        ...
        y_vals = [gr.GetY()[i] for i in range(gr.GetN())]
        format_histo(gr, x_tit='Time [min]', y_tit='Mean Pulse Height [au]', y_off=1.6, x_range=[self.Run.StartTime, self.Bins.get_time()[1][-1]],
                     t_ax_off=self.Run.StartTime if rel_t else 0, y_range=ax_range(min(y_vals), max(y_vals), .5, .5) if y_range is None else y_range, ndivx=505)
        fit = gr.Fit('pol0', 'qs')
        self.Draw(gr, draw_opt='ap', show=show)
    ...
    def show_ph_overview(self, binning=None):
        ...
        for i, h in enumerate([h1, h2], 1):
            ...
            h.Draw()
        self.Draw.save_plots('PHEvolutionOverview{0}'.format(self.Bins.Size))
        self.Draw.add(h2, c)
    ...
    def draw_signal_distribution(self, cut=None, evnt_corr=True, off_corr=False, show=True, sig=None, bin_width=.5, events=None,
        ...
        def func():
            ...
            values = self.get_root_vec(var=self.generate_signal_name(sig, evnt_corr, off_corr, cut), cut=self.Cut(cut), nentries=nentries, firstentry=start)
            # TODO: bin alrady in bins method
            h1 = self.Draw.distribution(values, self.Bins.get_pad_ph(bin_width), show=False, x_tit='Pulse Height [mV]', y_tit='Number of Entries', y_off=2, fill_color=Draw.FillColor)
    ...
    def draw_signal_vs_peaktime(self, region=None, cut=None, bin_size=None, fine_corr=False, prof=True, x=None, y=None, y_range=None, xbins=None, show=True):
        ...
        self.Draw(h, 'SignalVsPeakPos', show, lm=.11, draw_opt='' if prof else 'colz', rm=.03 if prof else .18)
        return h
    ...
    def draw_signal_vs_tot(self, show=True):
        ...
        m, s = mean_sigma(v)
        h = TH2F('hstot', 'Pulse Height vs. Time over Threshold', 200, *(ax_range(m - 3 * s, m + 3 * s, .3, .3) + self.Bins.get_pad_ph()))
        ...
        self.Draw(h, show=show, lm=.11, draw_opt='colz', rm=.15)
    ...
    def draw_tot_vs_peaktime(self, corr=True, fine_corr=False, bin_size=None, show=True):
        ...
        self.Draw(p, 'ToTVsPeakPos{}{}'.format(int(corr), int(fine_corr)), show, lm=.11)
    ...
    def draw_signal_vs_times(self, bin_size=.2, show=True):
        ...
        self.Draw(p, show=show, lm=.12, rm=.13, draw_opt='colz')
    ...
    def draw_signal_vs_triggercell(self, bin_width=10, cut=None, show=True):
        ...
        self.Draw(p, 'SignalVsTriggerCell', show, lm=.11)
    ...
    def show_bucket_histos(self):
        ...
        ped1_fit.SetParameters(*[fit.GetParameter(i) for i in range(3, 6)])
        ped2_fit.SetParameters(*[fit.GetParameter(i) for i in range(6, 9)])
        h_sig = deepcopy(h)
        h_ped1 = deepcopy(h)
        h_ped2 = deepcopy(h)
        ...
        for i, h in enumerate([h_ped1, h_ped2, h_sig]):
            h.SetStats(0)
            h.SetLineColor(self.Draw.get_color(3))
            h.SetLineWidth(2)
            h.Draw('same') if i else h.Draw()
        self.Draw.save_plots('BucketHistos')
        Draw.add(h, h_sig, h_ped1, h_ped2, c)
    ...
    def show_bucket_numbers(self, show=True):
        ...
        def func():
            ...
            if show:
                print('New Bucket: {0} / {1} = {2:4.2f}%'.format(n_new, self.Run.NEvents, n_new / float(self.Run.NEvents) * 100))
                print('Old Bucket: {0} / {1} = {2:4.2f}%'.format(n_old, self.Run.NEvents, n_old / float(self.Run.NEvents) * 100))
            return {'old': n_old, 'new': n_new, 'all': float(self.Run.NEvents)}
    ...
    def show_bucket_hits(self, show=True):
        ...
        for i, proj in enumerate(projections):
            last_bin = None
            for bin_ in range(proj.GetNbinsX()):
    ...
    def draw_bucket_waveforms(self, show=True, t_corr=True, start=100000):
        ...
        self.Draw.save_plots('BucketWaveforms')
    ...
    def show_bucket_means(self, show=True, plot_histos=True):
        ...
        def func():
            ...
            for key, value in self.Cut.CutStrings.items():
            ...
            if plot_histos:
                c = TCanvas('c', 'Bucket Histos', 1000, 1000)
                format_histo(h1, color=self.Draw.get_color(), lw=1, x_tit='Pulse Height [au]', y_tit='Entries')
                h1.Draw()
                format_histo(h2, color=self.Draw.get_color(), lw=1)
                h2.Draw('same')
                format_histo(h3, color=self.Draw.get_color(), lw=1)
                h3.Draw('same')
                Draw.add([h1, h2, h3, c])
    ...
    def compare_single_cuts(self):
        ...
        for key, value in self.Cut.CutStrings.items():
            if str(value) or key == 'raw':
                ...
                self.Draw.save_plots(save_name)
                # draw all single plots into c2
                c2.cd()
                histo.SetLineColor(self.Draw.get_color())
                if not drawn_first:
    ...
    def compare_normalised_cuts(self, scale=False, show=True):
        ...
        entries = 0
        for value in self.Cut.CutStrings.values():
            if str(value):
                entries += 1
        legend = Draw.make_legend(x1=.57, nentries=entries - 2)
        histos = []
        for key, value in self.Cut.CutStrings.items():
            if str(value) or key == 'raw':
                ...
                self.Draw.save_plots(save_name)
                # draw all single plots into c2
                histo.SetLineColor(self.Draw.get_color(len(self.Cut.CutStrings.items())))
        ...
        self.Draw(stack, name, show, lm=.15, leg=legend, draw_opt='nostack')
        gROOT.ProcessLine("gErrorIgnoreLevel = 0;")
        gROOT.SetBatch(0)
    ...
    def compare_consecutive_cuts(self, scale=False, show=True, save_single=True, short=False, x_range=None, redo=False):
        ...
        legend = Draw.make_legend(.75 if short else .71, .88, nentries=len(self.Cut.ConsecutiveCuts) + 1 if not short else len(short_cuts) + 1, scale=.7)
        stack = THStack('scc', 'Signal Distribution with Consecutive Cuts')
        leg_style = 'l' if scale else 'f'
        for i, (key, cut) in enumerate(self.Cut.ConsecutiveCuts.items()):
            ...
            self.Draw(h, 'signal_distribution_{n}cuts'.format(n=i), show=False, save=save_single)
            color = self.Draw.get_color(len(self.Cut.ConsecutiveCuts.items()))
        ...
        self.Draw(stack, save_name, show, leg=legend, draw_opt='nostack', lm=0.14)
        stack.SetName(stack.GetName() + 'logy')
        # stack.SetMaximum(stack.GetMaximum() * 1.2)
        self.Draw(stack, '{name}LogY'.format(name=save_name), show, logy=True, draw_opt='nostack', lm=0.14)

    def draw_cut_means(self, show=True, short=False):
        gr = Draw.make_tgrapherrors('gr_cm', 'Mean of Pulse Height for Consecutive Cuts')
        gr.SetPoint(0, 0, 0)
        short_keys = ['raw', 'saturated', 'timing', 'bucket', 'pulser', 'tracks', 'fiducial']
        cuts = OrderedDict((key, item) for key, item in self.Cut.ConsecutiveCuts.items() if not short or key in short_keys)
        for i, (key, cut) in enumerate(cuts.items(), 1):
            self.info('adding cut {0}'.format(key))
        ...
        y = [gr.GetY()[i] for i in range(1, gr.GetN())]
        gr.GetYaxis().SetRangeUser(min(y) - 1, max(y) + 1)
        gr.GetXaxis().SetLabelSize(.05)
        for i in range(1, gr.GetN()):
            bin_x = gr.GetXaxis().FindBin(i)
            gr.GetXaxis().SetBinLabel(bin_x, list(cuts.keys())[i - 1])
        Draw.add(self.Draw(gr, 'CutMeans{s}'.format(s='Short' if short else ''), show, bm=.25, draw_opt='bap', lm=.12, x=1.5))
        gROOT.ProcessLine('gErrorIgnoreLevel = 0;')
    ...
    def draw_distance_vs_ph(self, show=True, bins=10):
        ...
        self.Draw(p, 'PHTrackLength', show, lm=.11)

    def test_landau_stats(self):
        gr = Draw.make_tgrapherrors('gr_ls', 'Landau Statistics')
        set_root_output(False)
        self.PBar.start(sum(int(pow(2, i / 2.)) for i in range(1, 40)))
        k = 0
        for j, i in enumerate(range(1, 40)):
            h = TH1F('h', 'h', 500, 0, 1000)
            for _ in range(int(pow(2, i / 2.))):
                k += 1
        ...
        self.PBar.finish()
        self.Draw(gr, draw_opt='alp', logx=True)

    def find_conv(self):
        gr = Draw.make_tgrapherrors('gr_c', 'chi2 vs nconv')
        for i, j in enumerate(range(10, 70, 5)):
            print(j)
    ...
    def draw_signal_vs_signale(self, show=True):
        ...
        Draw.add(self.Draw(h, 'SignalEvsSignalB', show, rm=.15, lm=.13, draw_opt='colz'))
        gStyle.SetPalette(1)
    ...
    def draw_alignment(self, n_pulser=200, thresh=40, show=True):
        ...
        leg = Draw.make_legend(nentries=2, x2=.9, margin=.2)
        leg.AddEntry(Draw.box(0, 0, 0, 0, line_color=3, name='b1'), 'aligned', 'f')
        leg.AddEntry(Draw.box(0, 0, 0, 0, line_color=2), 'misaligned', 'f')
        self.Draw(h, 'EventAlignment', draw_opt='col', rm=.08, leg=leg, show=show, prnt=show)
        return h
    ...
    def draw_snrs(self, show=True, lego=True, proj=False, draw_opt='lego2'):
        ...
        gr = Draw.make_tgrapherrors('gr', 'Signal to Noise Ratios')
        h = TProfile2D('h_snr', 'Signal to Noise Ratios', 10, 0, 5, 10, 0, 5)
        i = 0
        for name, region in self.get_all_signal_names().items():
            if self.SignalRegionName.split('_')[-1] in region:
                ...
                gr.GetListOfFunctions().Add(Draw.tlatex(i + 1, snr.n + snr.s * 1.5, str(peak_integral), align=22, size=.02))
                i += 1
        format_histo(gr, y_tit='SNR', y_off=1.2, color=self.Draw.get_color(), fill_color=1)
        vals = sorted([h.GetBinContent(i) for i in range(h.GetNbinsX() * h.GetNbinsY()) if h.GetBinContent(i)])
        ...
        if draw_opt == 'coltext':
            ...
            self.Draw(h, 'SNRLego', show and lego, draw_opt=draw_opt, bm=.2, rm=.1, lm=.13, phi=-30, theta=40)
        gStyle.SetPalette(1)
        self.Draw(gr, 'SNR', not (lego or proj) and show, draw_opt='bap')

    def show_best_snr(self, histo, x, y, show):
        h = histo
        format_histo(h, x_off=1, y_off=1.15, stats=0, z_tit='snr [au]', z_off=1.35)
        self.Draw(h, '', show, draw_opt='colztext', rm=.16)
        Draw.vertical_line(x, -1e5, 1e5, color=2, style=2, name='a', w=2)
        Draw.vertical_line(x + .5, -1e5, 1e5, color=2, style=2, name='b', w=2)
        Draw.horizontal_line(y, 0, 10, color=418, style=2, w=2, name='c')
        Draw.horizontal_line(y + .5, 0, 100, color=418, style=2, w=2, name='d')
        self.Draw.save_plots('SNRColText')

    def __draw_profiles(self, histo, x, y, show=True):
        h = histo
        py = h.ProfileY('Right Length', x, x)
        px = h.ProfileX('Left Length', y, y)
        vals = [py.GetBinContent(i) for i in range(py.GetNbinsX()) if py.GetBinContent(i)] + [px.GetBinContent(i) for i in range(px.GetNbinsX()) if px.GetBinContent(i)]
        format_histo(py, stats=0, lw=2)
        format_histo(px, stats=0, lw=2)
        py.SetLineColor(2)
        px.SetLineColor(418)
        leg = Draw.make_legend(.68, .95)
        ...
        stack.SetMinimum(ax_range(min(vals), max(vals), .5, .5)[0])
        stack.SetMaximum(ax_range(min(vals), max(vals), .5, .5)[1])
        self.Draw(stack, 'SNRProfiles', show, draw_opt='nostack', leg=leg, lm=.13)
    ...
    def find_best_snr(self, show=True, same_width=False):
        gROOT.SetBatch(1)
        gr = Draw.make_tgrapherrors('gr', 'Signal to Noise Ratios')
        peak_integrals = OrderedDict(sorted({key: value for key, value in self.Run.peak_integrals.items() if len(key) < 3}.items()))
        i = 0
        for name, value in peak_integrals.items():
        ...
        gr.Draw('ap')
        gROOT.SetBatch(0)
        self.Draw.save_plots('BestSNR')
        Draw.add([gr, c])

    def signal_vs_peakintegral(self, show=True, ped=False):
        gROOT.SetBatch(1)
        gr = Draw.make_tgrapherrors('gr', '{sig} vs Peak Integral'.format(sig='Signal' if not ped else 'Pedestal'))
        peak_integrals = OrderedDict(sorted({key: value for key, value in self.Run.peak_integrals.items() if len(key) < 3}.items()))
        i = 0
        ...
        gr.Draw('ap')
        gROOT.SetBatch(0)
        self.Draw.save_plots('{sig}PeakInt_{rat}'.format(rat=ratio, sig='Ped' if ped else 'Sig'))
        Draw.add([gr, c])
        ...
        for region in self.Run.IntegralRegions[self.DUT.Number - 1]:
            if sig_type in region:
                for integral in self.Run.PeakIntegrals[self.DUT.Number - 1]:
                    ...
                    names[self.SignalDefinition.format(pol=self.PulserPolarity if sig_type == 'pulser' else self.Polarity, num=num)] = reg
        return names
