...
from src.PulserCollection import PulserCollection
from src.analysis_collection import *
from src.pad_analysis import PadAnalysis
from helpers.draw import scale_multigraph, format_statbox, format_histo, get_hist_vecs
...
class PadCollection(AnalysisCollection):
    ...
    def print_all_off_results(self):
        ...
        for ana in list(self.Analysis.values()):
    ...
    def draw_ph_with_currents(self, show=True, scale=1):
        ...
        legends = [Draw.make_legend(*positions[i], nentries=entries[i], scale=2.5 * (27 / 36. if i == 2 else 1), x2=lm + .22) for i in range(3)]
        dummy_gr = Draw.make_tgrapherrors('g', 'g', width=2)
        ...
        y_ranges = [ax_range(scale_margins(pul, ph), .3), ax_range(scale_margins(ph, pul), .3), cur_range]
        ...
        c = Draw.canvas('c_phc', 'c', 1.5, 1, transp=True, show=show)
        ...
        for i, gr in enumerate([pul, ph, cur]):
            ...
            Draw.tpad('p{}'.format(i), 'p{}'.format(i), pos=[0, ypos[i][0], 1, ypos[i][1]], gridx=True, gridy=True, margins=margins[i], transparent=True)
            gr.GetXaxis().SetLimits(*x_range)
            gr.Draw(draw_opts[i])
            legends[i].Draw()
            c.cd()

        self.Draw.save_plots('PhPulserCurrent', all_pads=False)
        Draw.add([ph, cur, pul, c, legends])

    def draw_pedestals(self, vs_time=False, sigma=False, pulser=False, redo=False, show=True):
        y_tit = 'Noise' if sigma else 'Pedestal'
        g = Draw.make_tgrapherrors('gps', '{} {}'.format('Pulser ' if pulser else '', y_tit), x=self.get_x_var(vs_time), y=self.get_pedestals(pulser, redo=redo)[1 if sigma else 0])
        ...
        self.Draw(g, save_name=save_name, show=show, logx=False if vs_time else True, lm=.12, draw_opt='ap')
        return g
    ...
    def draw_ped_spread(self, pulser=False, redo=False, show=True):
        ...
        self.Draw(h, 'PedestalSpread', lm=.11, show=show)
    ...
    def draw_noise_spread(self, pulser=False, redo=False, show=True):
        ...
        self.Draw(h, 'NoiseSpread', lm=.11, show=show)
    ...
    def draw_snrs(self, flux=True, draw=True):
        ...
        gr = Draw.make_tgrapherrors('gr', 'SNR vs {mode}'.format(mode=mode))
        i = 0
        for key, ana in list(self.Analyses.items()):
            snr = ana.calc_snr()
        ...
        if flux:
            c.SetLogx()
        gr.Draw('ap')
        self.Draw.save_plots('AllSNRs', canvas=c)
        Draw.add([gr, c])
        gROOT.SetBatch(0)
    ...
    def draw_signal_legend(self):
        ...
        l1 = Draw.make_legend(.17, .88, nentries=2, margin=.05, clean=True, x2=.57, cols=2)
    ...
    def compare_pedestals(self, show=True):
        ...
        legend.Draw()
        gROOT.SetBatch(0)
        self.Draw.save_plots('PulserPedestalComparison')
        Draw.add([c, graphs, legend])
    ...
    def compare_signal_vs_peak_height(self, i0=0, i1=-1, ym=.05, cft=False, show=True, redo=False):
        ...
        g = Draw.make_tgrapherrors('gcspt', 'Signal Ratio Vs Peak Time at {} and {}'.format(flux0, flux1), x=x, y=y)
        format_histo(g, x_tit='Signal Peak Time [ns]', y_tit='Signal Ratio', y_off=1.7, y_range=array([-ym, ym]) + 1)
        self.Draw(g, show=show, lm=.13, gridy=True)
        y = y[y > .1]
        m, s = mean_sigma(y)
    ...
    def compare_all_sig_vs_peakheight(self, ym=.05, show=True):
        ...
        for i in range(1, self.NRuns):
            ...
            self.PBar.update()
        g = Draw.make_tgrapherrors('gasphr', 'Signal Ratio Vs Flux', x=self.get_fluxes()[1:], y=values)
        format_histo(g, y_tit='Signal Ratio', y_off=1.7, y_range=array([-ym, ym]) + 1, **self.get_x_args(False))
        self.Draw(g, show=show, lm=.13, logx=True, gridy=True)
    ...
    def draw_bucket_info(self, flux=True, show=True, mean_=True):
        ...
        mode = 'Flux' if flux else 'Run'
        gr1 = Draw.make_tgrapherrors('gr1', '', color=self.Draw.get_color())
        prefix = 'Number of Bucket Cut Events' if not mean_ else 'Mean Pulse Height with Different Bucket Cuts'
        gr2 = Draw.make_tgrapherrors('gr2', '{pref} vs {mod}'.format(pref=prefix, mod=mode), color=self.Draw.get_color())
        gr3 = Draw.make_tgrapherrors('gr3', '', color=self.Draw.get_color())
        i = 0
        for key, ana in list(self.Analyses.items()):
            x = ana.Run.Flux if flux else key
            if not mean_:
        ...
        self.Draw.save_plots('{mode}_' + mode)
        Draw.add([c, gr1, gr2, gr3, leg])
    # endregion CUTS
    # ----------------------------------------
    ...
    def draw_peak_timings(self, vs_time=False, redo=False, show=True):
        ...
        g = Draw.make_tgrapherrors('gr', 'Peak Times', x=self.get_x_var(vs_time), y=self.get_values('peak timings', self.Analysis.get_peak_timing, redo=redo))
        format_histo(g, y_tit='Peak Time [ns]', y_off=1.9, **self.get_x_args(vs_time))
        self.Draw(g, 'PeakTimes', lm=.14, logx=not vs_time, show=show)
    ...
    def show_peak_distribution(self, show=True):
        ...
        for run, ana in list(self.Analyses.items()):
            if ana.IsAligned:
        ...
        if show:
            gROOT.SetBatch(0)
        for i, histo in enumerate(list(h.values()), 1):
            c.cd(i)
            histo.Draw('col')
            ex[i - 1].Draw()
            histo.Draw('colz same')
        self.Draw.save_plots('PeakDistribution')
        gROOT.ProcessLine('gErrorIgnoreLevel = 0;')
        gROOT.SetBatch(0)
        Draw.add([c, ex])
    # endregion TIMING
    ...
    def save_additional(self):
        for i, ana in enumerate(self.get_analyses()):
            ...
            self.Draw.save_plots('', 'r{}'.format(i), res_dir='', ftype='pdf')
    ...
    def draw_peaks_vs_rate(self, normalise=False, log_=True, show=True):
        ...
        g = Draw.make_tgrapherrors('gpr', 'Number of Additional Peaks vs Flux', x=fluxes, y=peak_heights)
        if not normalise:
            format_statbox(fit=True, x=.52)
            g.Fit('pol1', 'qs')
        format_histo(g, y_tit='Number of Additional Peaks {}'.format('/ Flux' if normalise else ''), y_off=1.5, **self.get_x_args(False))
        self.Draw(g, show=show, lm=.12, logy=log_ and not normalise, logx=log_)
    ...
    def compare_fluxes(self, normalise=False, fit=True, log_=True, corr=False, avrg=False, redo=False, y_range=None, show=True):
        ...
        g = Draw.make_tgrapherrors('gff', 'FAST-OR Flux vs Peak Flux', x=f0, y=f1)
        ...
        if fit:
            format_statbox(only_fit=True, w=.2, x=.5)
            g.Fit('pol1', 'qs')
        self.Draw(g, show=show, lm=.12, logx=log_, logy=log_ and not normalise)
    ...
    def draw_bunch_systematics(self, bunch=0, show=True):
        ...
        g = Draw.make_tgrapherrors(self.get_fluxes(), y=single_bunch / all_bunches, title='Systematics of the Number of Peaks of Bunch {}'.format(bunch))
        format_histo(g, y_tit='N Peaks in Bunch {} / Average N Peaks per Bunch'.format(bunch), y_off=1.3, **self.get_x_args(False))
        self.Draw(g, lm=.12, show=show, logx=True)
    # endregion PEAKS
    # ----------------------------------------
    ...
    def draw_flux_comparison(self):
        g1 = Draw.make_tgrapherrors('g_fp', 'Number of Peaks', color=self.Draw.get_color())
        pixel_fluxes = [ana.Run.Flux / 1000. for ana in list(self.Analyses.values())]
        g2 = Draw.make_tgrapherrors('g_ff', 'Pixel Fast-OR', x=self.Runs, y=pixel_fluxes, color=self.Draw.get_color())
        for i, (run, ana) in enumerate(self.Analyses.items()):
        ...
        mg.Add(g1, 'pl')
        mg.Add(g2, 'pl')
        leg = Draw.make_legend(nentries=3, x2=.4)
        ...
        self.Draw(mg, 'FluxComparison', draw_opt='a', lm=.12, bm=.2, leg=leg)
