...
class PedestalAnalysis(SubAnanlysis):
    def __init__(self, pad_analysis):
        super().__init__(pad_analysis)
    ...
    def draw_signal_pedestal(self, name=None, show=True):
        ...
        format_histo(h, x_tit='Pedestal [mV]', y_tit='Number of Entries', y_off=1.8, fill_color=Draw.FillColor)
    ...
    def draw_disto(self, name=None, cut=None, logy=False, show=True, save=True, redo=False, prnt=True, normalise=None):
        ...
        format_histo(h, 'Pedestal', x_tit='Pedestal [mV]', y_tit='Number of Entries', y_off=1.8, fill_color=Draw.FillColor, normalise=normalise)
        set_drawing_range(h, rfac=.2)
        self.Draw(h, 'PedestalDistribution{}'.format(cut.GetName()), show, save=save, logy=logy, lm=.13, prnt=prnt)
        h.Sumw2(False)
        self.Histogram = h
        return h
    ...
    def draw_disto_fit(self, name=None, cut=None, logy=False, show=True, save=True, redo=False, prnt=True, draw_cut=False, normalise=None):
        ...
        self.Draw(h, '', show, logy=logy, lm=.13, prnt=prnt)
        if draw_cut:
            b = self.__draw_cut(h)
            h.Draw('same')
            b.Draw('l')
        ...
        self.Histogram = h
        SaveDraw.server_pickle(picklepath, fit_pars)
        return fit_pars
    ...
    def draw_sigma_selection(self, show=True, redo=False):
        ...
        g = Draw.box(x[0], -1e9, x[1], 1e9, line_color=Draw.FillColor, width=2, fillstyle=3001, show=False)
        l1 = Draw.make_legend(.7, .63, nentries=3)
        ...
        g.Draw('f')
        g.Draw('l')
        self.Draw(h, 'PedSigmaSelection', show=show, logy=True, leg=l1, canvas=gROOT.GetListOfCanvases()[-1], draw_opt='same')
        # self.Draw.save_plots('PedSigmaSelection', show=show)
    ...
    def draw_signal_time(self, signal_name=None, rel_t=False, show=True):
        ...
        self.Draw(h, 'PedestalTime', show, lm=.12, draw_opt='colz', rm=.15)
        return h
    ...
    def draw_pulse_height_histo(self, signal_name=None, show=True, sigma=False, redo=False):
        ...
        def func():
            ...
            g1 = Draw.make_tgrapherrors('g_pph', 'Pedestal{s} Pulse Height Evolution'.format(s=' Sigma' if sigma else ''))
        ...
        self.Draw(g, 'Pedestal{s}PulseHeight'.format(s='Sigma' if sigma else ''), show=show, lm=.14, draw_opt='apl')
        return g
    ...
    def fit_pulse_height(self, signal_name=None, show=True, sigma=False):
        ...
        fit = g.Fit('pol0', 'qs')
        self.Draw(g, '{n}Fit'.format(n=g.GetTitle()), show, lm=.12, draw_opt='apl')
        return FitRes(fit)
    ...
    def compare(self):
        g = Draw.make_tgrapherrors('g_cp', 'Pedestal Comparison')
        ...
        self.Draw(g, 'PedestalComparison', lm=.12)
