...
from helpers.draw import format_histo, Draw, format_statbox
from helpers.utils import make_ufloat
...
class VoltageScan(Analysis):
    ...
    def get_voltages(self):
        return [make_ufloat((ana.Bias, abs(ana.Bias * 1e-4))) for ana in self.Analyses.values()]
    ...
    def draw_pedestals(self, cut=None, pulser=False, sigma=False, show=True, redo=False):
        ...
        y_values = [dic['sigma' if sigma else 'ph'] for dic in pedestals.values()]
        gr = Draw.make_tgrapherrors('g_vpd', '{p}Pedestal {m} vs. Bias Voltage'.format(p='Pulser' if pulser else '', m=mode), x=self.get_voltages(), y=y_values, color=self.Draw.get_color())
        format_histo(gr, x_tit='Voltage [V]', y_tit='Pulse Height [au]', y_off=1.4)
        self.Draw(gr, '{s}Pedestal{m}Voltage'.format(s='Pulser' if pulser else '', m=mode), show, lm=.13, draw_opt='ap')

    def draw_efficiency(self, show=True):
        x = self.get_voltages()
        efficiencies = [ana.get_hit_efficiency() for ana in self.Analyses.values()]
        g = Draw.make_tgrapherrors('gev', 'Efficiency vs. Voltage', x=x, y=efficiencies, color=Draw.Colors[2])
        format_histo(g, x_tit='Voltage [V]', y_tit='Hit Efficiency [%]', y_off=1.3, y_range=[0, 108], x_range=[min(x).n - 5, max(x).n + 5], markersize=1.3)
        self.Draw(g, show=show, gridy=True, draw_opt='pa')
        Draw.preliminary()
        self.Draw.save_plots('EfficiencyVoltage')
    ...
    def draw_pulse_height(self, binning=None, pulser=False, redo=False, show=True):
        ...
        y_values = [dic['ph'] for dic in ph.values()]
        x = self.get_voltages()
        ...
        g = Draw.make_tgrapherrors('gStatError', 'stat. error', Draw.Colors[2], x=x, y=y_values, marker_size=1)
        ...
        self.Draw(g, draw_opt='apl', lm=.14, show=show)
        Draw.preliminary()
        self.Draw.save_plots('{s}VoltageScan'.format(s='Signal' if not pulser else 'Pulser'))
        return g

    def draw_slope(self, gr=False, show=True):
        h = TH1F('hSV', 'PH Slope Distribution', 10, -1, 1) if not gr else Draw.make_tgrapherrors('gSV', 'PH Slope vs. Voltage')
        ...
        self.Draw(h, show=show, draw_opt='alp' if gr else '')
