...
class PadAnalysis(Analysis):
	...
	def draw_signal_map(self, res=1.5, cut=None, fid=False, hitmap=False, redo=True, show=True, prnt=True, z_range=None, save=True):
	...
	def draw_signal_map_s30(self, res=1.5, cut=None, fid=False, hitmap=False, redo=True, show=True, prnt=True, z_range=[0,16000], save=True):
		cut = self.Cut.generate_special_cut(excluded=['fiducial'], prnt=prnt) if not fid and cut is None else cut
		cut = self.Cut.all_cut if cut is None else TCut(cut)
		suf = '{c}_{ch}_{res}'.format(c=cut.GetName(), ch=self.Cut.CutConfig['chi2X'], res=res)
		pickle_path = self.make_pickle_path('SignalMaps', 'Hit' if hitmap else 'Signal', run=self.RunNumber, ch=self.DiamondNumber, suf=suf)
		...
		def func():
			...
			h1 = TH2I(*atts) if hitmap else TProfile2D(*atts)
			self.log_info('drawing {mode}map of {dia} for Run {run}...'.format(dia=self.DiamondName, run=self.RunNumber, mode='hit' if hitmap else 'signal '), prnt=prnt)
			sig = self.generate_signal_name(off_corr=True)
			x_var, y_var = (self.Cut.get_track_var(self.DiamondNumber - 1, v) for v in ['x', 'y'])
			self.tree.Draw('1744.0*{z}{y}*10:{x}*10>>{h}'.format(z=sig + ':' if not hitmap else '', x=x_var, y=y_var, h=name), cut, 'goff')
			return h1
		...
		self.set_dia_margins(h)
		self.set_z_range(h)
		z_tit = 'Number of Entries' if hitmap else '#eh-pairs []'
		self.format_histo(h, x_tit='Track Position X [mm]', y_tit='Track Position Y [mm]', y_off=1.4, z_off=2, z_tit=z_tit, ncont=50, ndivy=510, ndivx=510, z_range=z_range)
		self.set_statbox(entries=True, x=0.82)
	...
	def draw_signal_map_s108(self, res=1.5, cut=None, fid=False, hitmap=False, redo=True, show=True, prnt=True, z_range=[0,16000], save=True):
		cut = self.Cut.generate_special_cut(excluded=['fiducial'], prnt=prnt) if not fid and cut is None else cut
		cut = self.Cut.all_cut if cut is None else TCut(cut)
		suf = '{c}_{ch}_{res}'.format(c=cut.GetName(), ch=self.Cut.CutConfig['chi2X'], res=res)
		pickle_path = self.make_pickle_path('SignalMaps', 'Hit' if hitmap else 'Signal', run=self.RunNumber, ch=self.DiamondNumber, suf=suf)
	...
	def generate_signal_name(self, signal=None, evnt_corr=True, off_corr=False, bin_corr=False, cut=None, region=None):
		...
		if bin_corr:
			...
			ped_fit = self.Pedestal.draw_disto_fit(cut=cut, save=False, show=False, prnt=False)
	...
	def draw_pulse_height_vs_eventnr(self, bin_size=None, cut=None, y_range=None, redo=False, corr=True, sig=None, rel_t=True, show=True, save=True, prnt=True):
		...
		signal = self.generate_signal_name(self.SignalName if sig is None else sig, corr)
		prof = TProfile()
		self.tree.Draw('{sig}:Entry$>>pph'.format(sig=signal), cut_str)
