...
from numpy import concatenate, zeros, sign, exp
import numpy as np
...
class AnalysisCollection(Elementary):
	...
	def draw_all(self):
		...
		self.draw_signal_maps()
		self.draw_signal_maps(hitmap=True)
		self.set_verbose(old_verbose)
		print
	...
	def pol1(self, x, offset, constant):
		return offset + constant*x

	def get_pulse_height_graph(self, binning=None, vs_time=False, first_last=True, redo=False, legend=True, scaling=1):

		marker_size = 1
		gStyle.SetEndErrorSize(4)
		ph = self.get_pulse_heights(binning, redo)
		y_values = [dic['ph']*scaling for dic in ph.itervalues()]
		...
		print x_values
		xval = [np.log(e.n) for e in x_values]
		yval =  [e.n for e in y_values]
		avg_y = np.mean(yval)
		yerr = [e.s for e in y_values]
 		p0=[18000, -0.4] #offset, constant
		params, extras = curve_fit(self.pol1, xval, yval, p0=p0, sigma=yerr, absolute_sigma=True)
		ffunc = self.pol1(np.array(xval), *params)
		print params[1]/avg_y, np.sqrt(extras[1][1])/avg_y
		plt.ion()
		plt.plot(xval, yval)
	...
	def draw_pulse_heights(self, binning=None, vs_time=False, show=True, show_first_last=True, save_comb=True, y_range=None, redo=False, scaling=1):

		mode = 'Time' if vs_time else 'Flux'
		mg = self.get_pulse_height_graph(binning, vs_time, show_first_last, redo, scaling=scaling)

		# small range
		self.format_histo(mg, color=None, x_tit='Time [hh:mm]' if vs_time else 'Flux [kHz/cm^{2}]', y_tit='Pulse Height [#eh-pairs]', y_off=1.75, x_off=1.3, draw_first=True,
	...
	def draw_signal_maps_s30(self, hitmap=False, redo=True):
		f = PadAnalysis.draw_dia_hitmap if hitmap else PadAnalysis.draw_signal_map_s30
		histos = self.make_plots('{} maps'.format('hit' if hitmap else 'signal'), f, {'show': False, 'prnt': False, 'redo': redo})
		#glob_max = (int(max([h.GetMaximum() for h in histos])) + 5) / 5 * 5
		#glob_min = int(min([h.GetMinimum() for h in histos])) / 5 * 5
	...
	def draw_signal_maps_s108(self, hitmap=False, redo=True):
		f = PadAnalysis.draw_dia_hitmap if hitmap else PadAnalysis.draw_signal_map_s108
		histos = self.make_plots('{} maps'.format('hit' if hitmap else 'signal'), f, {'show': False, 'prnt': False, 'redo': redo})
		#glob_max = (int(max([h.GetMaximum() for h in histos])) + 5) / 5 * 5
		#glob_min = int(min([h.GetMinimum() for h in histos])) / 5 * 5
